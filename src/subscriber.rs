...
use crate::{IPV4, IPV6, PORT, join_multicast, new_socket};
...
impl MulticastSubscriber {
    ...
    /// ```
    pub fn new_with_interface(addr: IpAddr, port: Option<u16>, interface: Option<&str>, buffer_size: Option<usize>) -> io::Result<Self> {
        ...
        Ok(MulticastSubscriber { socket, addr, buffer_size, interface: interface.map(String::from) })
    }
    ...
    /// ```
    pub fn new_ipv4_with_interface(port: Option<u16>, interface: Option<&str>, buffer_size: Option<usize>) -> io::Result<Self> {
    ...
    /// ```
    pub fn new_ipv6_with_interface(port: Option<u16>, interface: Option<&str>, buffer_size: Option<usize>) -> io::Result<Self> {
    ...
    pub fn receive_batch(
        ...
        if results.is_empty() && max_wait_time.is_some() && start_time.elapsed() >= max_wait_time.unwrap() {
            return Err(io::Error::new(io::ErrorKind::TimedOut, "Timed out waiting for batch messages"));
        }
...
mod tests {
    ...
    fn test_subscriber_ipv4() {
        ...
        // Create subscriber
        let subscriber = MulticastSubscriber::new_ipv4(Some(port), None)
        ...
        let sender_thread = thread::spawn(move || {
            ...
            // Send a test message
            socket.send_to(b"Hello from test!", &addr).expect("Failed to send message");
        });
    ...
    fn test_receive_and_respond() {
        ...
        let sender_thread = thread::spawn(move || {
            ...
            // Send a test message
            socket.send_to(b"Request message", &addr).expect("Failed to send message");
        ...
        match subscriber.receive_and_respond(
            Some(Duration::from_secs(5)), 
    ...
    fn test_background_subscriber() {
        ...
        if !messages.is_empty() {
            ...
            println!("No messages received in background test - this may be expected on some platforms");
        }
    }
    ...
    fn test_subscriber_interface_creation() {
        ...
        match MulticastSubscriber::new_ipv4_with_interface(Some(port), Some("nonexistent_iface"), None) {
            Ok(_) => {
            ...
            match (
                ...
                (Ok(subscriber), Ok(publisher)) => {
                    ...
                    match publisher.publish(test_message) {
                        Ok(bytes_sent) => {
                            println!("Published {} bytes on interface {}", bytes_sent, thread_iface);
                        },
                        Err(e) => {
